...
class TestArrayPieces(unittest.TestCase):
    ...
    def test_array_pieces(self):
        ...
        for array_offset in sorted(array_pieces_results.keys()):
            ...
            assert piece_array.dtype.itemsize * reduce(lambda x, y: x * y / 16,
                                                       piece_array.shape) <= sixteenth_bytes, 'piece_array too large'
            ...
            slices = tuple([slice(array_offset[dim_index],
                                  array_offset[dim_index] + piece_array.shape[dim_index]
                                  ) for dim_index in range(2)])
            assert not np.any(
                piece_array - test_array[slices]), 'Array contents changed for array piece at %s' % array_offset
        ...
        for array_offset in sorted(array_pieces_results.keys()):
            ...
            expected_shape = tuple([test_array.shape[dim_index] / 4 + overlap
                                    if array_offset[dim_index] == 0
                                       or test_array.shape[dim_index] - array_offset[dim_index] - overlap <=
                                       test_array.shape[dim_index] / 4

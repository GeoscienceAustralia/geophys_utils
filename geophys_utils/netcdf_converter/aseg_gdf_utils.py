...
SIG_FIGS = OrderedDict([('uint8', 4),  # 128
                        ...
                        ('float32', 7),  # 7.2
                        ('float64', 35)
                        # 15.9 - should be 16, but made 35 to support unrealistic precision specifications
                        ]
                       )
...
DTYPE_REDUCTION_LISTS = [['int64', 'int32', 'int16', 'int8'],  # Integer dtypes
                         ...
                         ['float64', 'float32']
                         # , 'int16', 'int8'] # Floating point dtypes - do NOT try casting to integer types
                         ]
...
def decode_aseg_gdf_format(aseg_gdf_format):
    ...
    logger.debug(
        'aseg_gdf_format: {}, columns: {}, aseg_dtype_code: {}, width_specifier: {}, decimal_places: {}'.format(
            aseg_gdf_format,
            columns,
            aseg_dtype_code,
            width_specifier,
...
def variable2aseg_gdf_format(array_variable, decimal_places=None):
    ...
    if aseg_dtype_code == 'I':  # Integer
        ...
        if decimal_places is not None:
            ...
            _columns, _aseg_dtype_code, _integer_digits, decimal_places = decode_aseg_gdf_format(
                array_variable.aseg_gdf_format)
            decimal_places = min(decimal_places, abs(sig_figs - integer_digits))
            logger.debug('decimal_places set to {} from variable attribute aseg_gdf_format {}'.format(decimal_places,
                                                                                                      array_variable.aseg_gdf_format))
            ...
            logger.debug(
                'decimal_places set to {} from sig_figs {} and integer_digits {}'.format(decimal_places, sig_figs,
                                                                                         integer_digits))
        ...
        if aseg_dtype_code == 'F':  # Floating point notation
            python_format = '{' + ':>{:d}.{:d}f'.format(width_specifier,
                                                        decimal_places) + '}'  # Add 1 to width for decimal point
        else:  # Exponential notation for 'D' or 'E'
            python_format = '{' + ':>{:d}.{:d}E'.format(width_specifier,
                                                        decimal_places) + '}'  # Add 1 to width for decimal point

    elif aseg_dtype_code == 'A':  # String
        if hasattr(array_variable, 'aseg_gdf_format'):
            _columns, _aseg_dtype_code, width_specifier, decimal_places = decode_aseg_gdf_format(
                array_variable.aseg_gdf_format)
...
def fix_field_precision(data_array, current_dtype, decimal_places, no_data_mask=[], fill_value=None):
    ...
    '''
    logger.debug(
        'data_array: {}, current_dtype: {}, decimal_places: {}'.format(data_array, current_dtype, decimal_places))
...
difference_array\n{}\ndecimal_places: {}\ndifference count: {}\ndifference values: '.format(current_dtype,
                ...
                if np.any(np.logical_or((smaller_exponent != data_exponent),
                    ...
                    logger.debug(
                        'Maximum mantissa difference: {}'.format(np.nanmax(np.abs(data_mantissa - smaller_mantissa))))
                    logger.debug(
                        'Maximum exponent difference: {}'.format(np.nanmax(np.abs(smaller_exponent - data_exponent))))
                    aseg_gdf_format, dtype, columns, width_specifier, decimal_places, python_format = variable2aseg_gdf_format(
                        smaller_array, decimal_places)
                    ...
                    if fill_value is not None:
                        ...
                        if np.any(no_data_mask):
                            ...
                            if np.any(data_array[~no_data_mask] == reduced_precision_fill_value):
                                logger.debug('Reduced precision fill value of {} is ambiguous'.format(
...
def truncate(fill_value, data_array, no_data_mask, width_specifier, decimal_places):
    ...
    try:
        ...
        pattern = re.compile(
            '(-?)\d*?(\d{0,' + '{}'.format(integer_digits) + '}\.\d{0,' + '{}'.format(decimal_places) + '})')
        ...
        assert not np.any(
            data_array[~no_data_mask] == truncated_fill_value), 'Truncated fill value of {} is ambiguous'.format(
            truncated_fill_value)
        ...
        logger.debug('Unable to truncate fill value from {} to {} ({}). Keeping original value.'.format(fill_value,
                                                                                                        truncated_fill_value,
                                                                                                        e))
        return fill_value

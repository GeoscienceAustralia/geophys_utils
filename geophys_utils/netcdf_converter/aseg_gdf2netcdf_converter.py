...
class ASEGGDF2NetCDFConverter(ToNetCDFConverter):
    ...
    def __init__(self,
        ...
        def get_field_definitions():
            ...
            def parse_dfn_file(dfn_path):
                ...
                for line in dfn_file:
                    ...
                    for semicolon_split_string in [semicolon_split_string.strip() for semicolon_split_string in
                                                   line.split(';')]:
                        for colon_split_string in [colon_split_string.strip() for colon_split_string in
                                                   semicolon_split_string.split(':')]:
                            for comma_split_string in [comma_split_string.strip() for comma_split_string in
                                                       colon_split_string.split(',')]:
                                definition = [equals_split_string.strip() for equals_split_string in
                                              comma_split_string.split('=')]
                                if len(definition) == 2:
                                    key_value_pairs[definition[0]] = definition[1]
                                elif len(definition) == 1:
                                    positional_value_list.append(definition[0])

                    logger.debug('line: {}\nkey_value_pairs: {}\npositional_value_list: {}'.format(line.strip(),
                                                                                                   pformat(
                                                                                                       key_value_pairs),
                                                                                                   pformat(
                                                                                                       positional_value_list)))
                    ...
                    if key_value_pairs.get('RT') in ['', 'DATA'] and (positional_value_list
                        ...
                        long_name = key_value_pairs.get('NAME') or (
                            positional_value_list[2] if len(positional_value_list) >= 3 else None)
                        ...
                        variable_attribute_dict = {attribute_name: key_value_pairs.get(key.upper())
                                                   ...
                                                   self.settings['variable_attributes'].items()
                    ...
                    # Set CRS from projection name
                        ...
                        if (key_value_pairs.get('RT') == 'PROJ') and (
                                positional_value_list[0] == 'COORDSYS'):  # As per ASEG standard
                            projection_name = key_value_pairs.get('NAME')
                            if projection_name:
                                self.spatial_ref = get_spatial_ref_from_wkt(projection_name)
                                logger.debug(
                                    'CRS set from .dfn file COORDSYS COMMENT attribute {}'.format(projection_name))
                                break  # Nothing more to do
                        elif (key_value_pairs.get('RT') == 'PROJ') and (
                                positional_value_list[0] == 'PROJNAME'):  # Non-standard
                            projection_name = key_value_pairs.get('COMMENT')
                            if projection_name:
                                self.spatial_ref = get_spatial_ref_from_wkt(projection_name)
                                logger.debug(
                                    'CRS set from .dfn file PROJNAME NAME attribute {}'.format(projection_name))
                                break  # Nothing more to do
                        elif (key_value_pairs.get('RT') == 'PROJ') and (
                                positional_value_list[0] == 'DATUM'):  # Unprojected
                            projection_name = key_value_pairs.get('NAME')
                            if projection_name:
                                self.spatial_ref = get_spatial_ref_from_wkt(projection_name)
            ...
            if not self.spatial_ref:
                try:
                    field_definition = [field_definition for field_definition in self.field_definitions if
                                        field_definition['short_name'] == 'latitude'][0]
        ...
        def read_data_file():
            ...
            def create_nc_cache():
                ...
                '''
                self.nc_cache_path = os.path.join(TEMP_DIR,
                ...
                for field_definition in self.field_definitions:
                        ...
                        if override_definition:
                            ...
                            if override_dimensions:
                                if type(override_dimensions) == list:
                                    field_dimension_name = '_plus_'.join(
                                        override_dimensions)  # Multiple partial dimensions
                    ...
                    logger.debug('\tCreating cache variable {} with dtype {} and dimension(s) {}'.format(short_name,
                                                                                                         field_definition[
                                                                                                             'dtype'],
                                                                                                         field_dimensions))
            ...
            def read_fixed_length_fields(line):
                ...
                for field_definition in self.field_definitions:
                    ...
                    for _column_offset in range(columns):
                        ...
                        if not aseg_gdf_format.startswith(
                                'A') and ' ' in value_string.strip():  # Not a string field and has a space in the middle
                            value_string = re.match('\s*\S*', value_string).group(
                                0)  # Strip anything after non-leading whitespace character
                            end_char_index = start_char_index + len(
                                value_string)  # Adjust character offset for next column
                        ...
                        try:
                            ...
                            logger.warning(
                                'Unable to convert "{}" field value "{}" to type {}'.format(short_name, value_string,
                                                                                            dtype))
                ...
                if line_column_count != self.column_count:
                    logger.warning('Invalid number of columns found: Expected {}, found {}. Skipping line'.format(
                        self.column_count, line_column_count))
                    logger.debug('line: "{}"'.format(line))
                    return None
                return row_list
            ...
            def read_delimited_fields(line, delimiter_regex='\s+'):
                ...
                if len(value_list) != self.column_count:
                    logger.warning('Invalid number of columns found: Expected {}, found {}. Skipping line'.format(
                        self.column_count, len(value_list)))
                    logger.debug('line: "{}"'.format(line))
                    return None
                ...
                if match:  # format string starts with integers - 2D variable
                    ...
                    if default_dimension_names:  # Default dimension name(s) found from settings
                        ...
                        # ideally be re-written
                            ...
                            assert set(default_dimension_names) <= set(
                                self.dimensions.keys()), 'Dimensions {} not all defined in {}'.format(
                                default_dimension_names, self.dimensions.keys())
                            combined_dimension_size = sum([self.dimensions[key] for key in default_dimension_names])
                            assert combined_dimension_size == field_dimension_size, 'Incorrect dimension sizes. Expected {}, found {}'.format(
                            ...
                            for part_dimension_name in default_dimension_names:
                                ...
                                field_definition_index += 1
                                logger.debug('Inserting new part-dimension field definition for {} at index {}'.format(
                                    new_field_definition['short_name'], field_definition_index))
                        ...
                        if not self.dimensions.get(dimension_name):
                            ...
                            assert self.dimensions[
                                       dimension_name] == field_dimension_size, 'Invalid size for variable {}. Expected {}, got {}'.format(
                                field_name,
        ...
        def fix_all_field_precisions():
            ...
            for field_definition in self.field_definitions:
                ...
                if precision_change_result:
                    self.info_output('Datatype for variable {} changed from {} to {}'.format(short_name, dtype,
                                                                                             precision_change_result[
                                                                                                 1]))
    ...
    def get_raw_data(self, variable_name):
        ...
        try:
            ...
            try:  # Try part dimension with shared cache variable
                ...
                return self._nc_cache_dataset.variables[field_definition['cache_variable_name']][:,
                       field_definition['column_offset']:field_definition['column_offset'] + field_definition[
                           'columns']]
            except:
                return None
    ...
    def get_global_attributes(self):
        ...
        metadata_dict = {'title': 'Dataset read from ASEG-GDF file {}'.format(os.path.basename(self.dat_path)),
                         ...
                         'geospatial_vertical_min': np.min(self.get_raw_data('elevation')) or np.min(
                             self.get_raw_data('dtm')),
                         'geospatial_vertical_max': np.max(self.get_raw_data('elevation')) or np.min(
                             self.get_raw_data('dtm')),
    ...
    def variable_generator(self):
        ...
        for field_definition in self.field_definitions:
            ...
            if (field_definition['columns'] == 1) and (unique_value_count == 1) and (
                    index_point_counts[0] == raw_data.shape[0]):

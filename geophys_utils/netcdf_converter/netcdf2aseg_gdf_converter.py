...
class RowCache(object):
    ...
    def read_points(self, start_index, end_index, point_mask=None):
        ...
        def expand_line_lookup_variable(lookup_variable_name):
            ...
            '''
            lookup_variable = self.nc_dataset.variables[lookup_variable_name]
            assert lookup_variable.dimensions == ('line',), 'Variable "{}" has dimensions {}'.format(
                lookup_variable_name, lookup_variable.dimensions)
        ...
        for field_definition in self.field_definitions:
            ...
            if len(variable.shape) == 0:  # Scalar variable - repeat value for each point
                ...
                'line',)) and variable_name != 'line':  # "line" variable will be dealt with through "line_index" in above case
            ...
            for variable_name, variable in self.nc_dataset.variables.items():
                ...
                if (variable.shape
                        and not (
                                ...
                                or (variable.dimensions == ('line',) and variable_name != 'line')
                        # Line-indexed variables
                        )):
                    continue
                ...
                if variable_name.endswith('_index') | hasattr(variable, 'lookup'):  # Index variable found
                    ...
                    try:
                        ...
                        logger.warning(
                            'Unable to access lookup variable "{}" for index variable "{}"'.format(short_name,
                                                                                                   variable_name))
                        continue
                ...
                for field_definition in self.field_definitions:
                    ...
                    if fill_value is not None:
                        optional_attribute_list.append(
                            'NULL=' + field_definition['python_format'].format(fill_value).strip())
                    ...
                    if variable_attributes:
                        for aseg_gdf_attribute, netcdf_attribute in self.settings['variable_attributes'].items():
                            ...
                            if attribute_value is not None:
                                optional_attribute_list.append('{aseg_gdf_attribute}={attribute_value}'.format(
                                    aseg_gdf_attribute=aseg_gdf_attribute,

...
class CSWUtils(object):
    ...
    def get_date_filter(self, start_datetime=None, stop_datetime=None, constraint='overlaps'):
        ...
        if constraint == 'overlaps':
            start_filter = fes.PropertyIsLessThanOrEqualTo(propertyname='ows100:TempExtent_begin',
                                                           literal=stop_date_string)
            stop_filter = fes.PropertyIsGreaterThanOrEqualTo(propertyname='ows100:TempExtent_end',
                                                             literal=start_date_string)
        elif constraint == 'within':
            start_filter = fes.PropertyIsGreaterThanOrEqualTo(propertyname='ows100:TempExtent_begin',
                                                              literal=start_date_string)
            stop_filter = fes.PropertyIsLessThanOrEqualTo(propertyname='ows100:TempExtent_end',
                                                          literal=stop_date_string)
    ...
    def any_case_match_filters(self, propertyname, word_list):
        ...
        for word in word_list:
            ...
            if len(word_variant_set) == 1:  # Use single filter if no "or" required
                filter_list.append(
                    fes.PropertyIsLike(propertyname=propertyname, literal=word_variant_set.pop(), matchCase=False))
            else:
        ...
        for csw in self.csw_list:
            ...
            while startposition and record_count < max_total_records:
                ...
                for uuid in [uuid for uuid in csw.records.keys() if uuid not in uuid_list]:
                    ...
                    if record.bbox:
                        record_dict['bbox'] = [float(ordinate) for ordinate in
                                               [record.bbox.minx, record.bbox.miny, record.bbox.maxx, record.bbox.maxy]]
                        record_dict['bbox_crs'] = str(
                            record.bbox.crs) or 'urn:ogc:def:crs:EPSG::4283'  # Default to GDA94

                        # TODO: REMOVE HACK BELOW THIS LINE WHEN GEONETWORK CSW OUTPUT HAS BEEN FIXED
                        # print(record_dict['bbox'], record_dict['bbox_crs'])

                        if record_dict['bbox_crs'][
                            -1] == ')':  # Hack for invalid CRS code "urn:ogc:def:crs:EPSG::GDA94 (EPSG:4283)"
                            record_dict['bbox_crs'] = record_dict['bbox_crs'][:-1]
                        else:  # Hack for incorrect XY coordinate order in response
                            record_dict['bbox'] = [record_dict['bbox'][1], record_dict['bbox'][0],
                                                   record_dict['bbox'][3], record_dict['bbox'][2]]

                        # Hack to change incorrect(?) lower-right, upper-left points to lower-left, upper-right
                        record_dict['bbox'] = [min(record_dict['bbox'][::2]), min(record_dict['bbox'][1::2]),
                                               max(record_dict['bbox'][::2]), max(record_dict['bbox'][1::2])]
    ...
    def query_csw(self,
        ...
        if identifier_list:
            if len(identifier_list) == 1:
                fes_filter_list += [
                    fes.PropertyIsLike(propertyname='Identifier', literal=identifier_list[0], matchCase=False)]
            else:
                fes_filter_list.append(fes.Or(
                    [fes.PropertyIsLike(propertyname='Identifier', literal=identifier, matchCase=False) for identifier
                     in identifier_list]))

        if alt_identifier_list:
            if len(alt_identifier_list) == 1:
                fes_filter_list += [
                    fes.PropertyIsLike(propertyname='AlternateIdentifier', literal=alt_identifier_list[0],
                                       matchCase=False)]
            else:
                fes_filter_list.append(fes.Or(
                    [fes.PropertyIsLike(propertyname='AlternateIdentifier', literal=alt_identifier, matchCase=False) for
                     alt_identifier in alt_identifier_list]))
        ...
        if anytext_list:
            fes_filter_list += [fes.PropertyIsLike(propertyname='anyText', literal=phrase, matchCase=False) for phrase
                                in anytext_list]
        ...
        if titleword_list:
            fes_filter_list += [fes.PropertyIsLike(propertyname='title', literal=titleword, matchCase=False) for
                                titleword in titleword_list]
            ...
            for file_distribution in record_dict['distributions']:
                if 'file' in file_distribution['protocol'].lower():
                    ...
                    try:
                        ...
                        if os.path.isfile(file_distribution['url']) and netCDF4.Dataset(
                                file_distribution['url']):  # Test for valid netCDF file

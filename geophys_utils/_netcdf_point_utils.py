...
class NetCDFPointUtils(NetCDFUtils):
    ...
    def get_spatial_mask(self, bounds, bounds_wkt=None):
        ...
        def get_intersection_mask(points, geometry):
            ...
            while chunk_start_index < len(points):
                ...
                _x_values, x_indices, _x_intersection_indices = np.intersect1d(points.flatten()[0::2],
                                                                               intersection_points.flatten()[0::2],
                                                                               return_indices=True)
                _y_values, y_indices, _y_intersection_indices = np.intersect1d(points.flatten()[1::2],
                                                                               intersection_points.flatten()[1::2],
                                                                               return_indices=True)
        ...
        if isinstance(bounds, BaseGeometry):  # Process shapely (multi)polygon bounds
            ...
            logger.debug('{}/{} points found in initial bounding box intersection'.format(np.count_nonzero(mask),
                                                                                          len(coordinates)))
            ...
            assert len(
                bounds) == 4, 'Invalid bounds iterable: {}. Must be of form [<xmin>, <ymin>, <xmax>, <ymax>]'.format(
                bounds)

            # Transform and reshape to [xmin, ymin, xmax, ymax]]
            native_crs_bounds = np.array(
                transform_coords(np.array(bounds).reshape((2, 2)), bounds_wkt, self.wkt).reshape((4,)))
            ...
            bounds_half_size = abs(np.array(
                [native_crs_bounds[2] - native_crs_bounds[0], native_crs_bounds[3] - native_crs_bounds[1]])) / 2.0
    ...
    def get_lookup_mask(self,
        ...
        if lookup_variable_name:
            ...
            if (lookup_variable.shape == ()
                    or ((len(lookup_variable.shape) == 1) and (
                            lookup_variable.dtype == '|S1'))):  # Scalar or string array
    ...
    def copy(self,
        ...
        if var_list:
            expanded_var_list = list(set(
                ...
                ['line', 'line_index']
                # Always include line numbers (This really should be in an overridden function in NetCDFLineUtils)
            ))
        else:
    ...
    def set_global_attributes(self, compute_shape=False, clockwise_polygon_orient=False):
        ...
        try:
            ...
            logger.debug('Setting global geometric metadata attributes in netCDF point dataset with {} points'.format(
                self.netcdf_dataset.dimensions['point'].size))

            attribute_dict = dict(
                zip(['geospatial_lon_min', 'geospatial_lat_min', 'geospatial_lon_max', 'geospatial_lat_max'],

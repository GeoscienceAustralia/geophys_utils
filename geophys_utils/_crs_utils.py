...
def get_spatial_ref_from_wkt(wkt_or_crs_name):
    ...
    if not result:
        ...
        logger.debug(
            'UTM CRS determined using SpatialReference.SetWellKnownGeogCS({}) (zone{})'.format(modified_crs_name,
                                                                                               utm_zone))
        return spatial_ref
...
def get_coordinate_transformation(from_wkt, to_wkt):
    ...
    if not from_spatial_ref or not to_spatial_ref or from_spatial_ref.ExportToWkt() == to_spatial_ref.ExportToWkt():
        return None
...
def transform_coords(coordinates, from_wkt, to_wkt):
    ...
    while chunk_start_index < len(coordinate_array):
        chunk_end_index = min(chunk_start_index + POINT_CHUNK_SIZE, len(coordinates))
        logger.debug(
            'Transforming coordinates {} to {} of {}'.format(chunk_start_index, chunk_end_index - 1, len(coordinates)))
        transformed_coord_chunk_list.append(
            np.array(coord_trans.TransformPoints(coordinate_array[slice(chunk_start_index, chunk_end_index)]))[:, 0:2])

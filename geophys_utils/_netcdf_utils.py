        ...
        try:
            ...
            for variable_name, variable in filtered_variables.items():
                ...
                for dimension_index in range(len(variable.dimensions)):
                    ...
                    dest_dim_size[dimension_name] = np.count_nonzero(
                        dim_mask)  # Update sizes to take masks into account
            ...
            for variable_name, input_variable in filtered_variables.items():
                ...
                # Removing depcrecated keys
                [var_options.pop(key, None) for key in ("zlib", "szip", "zstd", "bzip2", "blosc")]
                ...
                if chunking and chunking != 'contiguous':
                    ...
                    input_variable_chunking = [
                        min(chunking[dimension_index], dest_dim_size[input_variable.dimensions[dimension_index]])
                ...
                if var_options.get('chunksizes'):
                    for dimension_index in range(len(input_variable.dimensions)):
                        var_options['chunksizes'][dimension_index] = min(
                            var_options['chunksizes'][dimension_index] or dest_dim_size[
                ...
                if variable_name not in empty_var_list:
                    ...
                    if input_variable.shape:  # array
                        ...
                        logger.debug('\tCopying {} array data of shape {}'.format(
                            variable_name,
                            tuple([input_variable_slices[dimension_index].stop - input_variable_slices[
                        ...
                        variable_masks = tuple(
                            [dim_mask_dict.get(dimension_name)  # Use specified mask if it exists, None otherwise
                        ...
                        if not input_variable_chunking:
                            ...
                            piece_counts = tuple([
                                int(math.ceil(float(
                                    self.netcdf_dataset.dimensions[input_variable.dimensions[dimension_index]].size) /
                            ...
                            piece_sizes[largest_piece_count_index] = min(
                                input_variable.shape[largest_piece_count_index],
                                int(smallest_piece_sizes[largest_piece_count_index] * (
                                            self.max_bytes // smallest_piece_bytes))
                                )

                            piece_bytes = np.dtype(input_variable.dtype).itemsize * reduce(lambda x, y: x * y,
                                                                                           piece_sizes)
                            ...
                            piece_index_ranges = [
                                (input_variable_slices[dimension_index].start // piece_sizes[dimension_index],
                                 int(math.ceil(
                                     float(input_variable_slices[dimension_index].stop) / piece_sizes[dimension_index]))
                                 )
                                for dimension_index in range(len(input_variable.dimensions))
                            ]
                            ...
                            logger.debug('\tCopying {} pieces of dimensions {}'.format(
                            ...
                            for piece_indices in itertools.product(
                                ...
                                offset_piece_indices = tuple([(indices[0] - indices[1] + 1)
                                                              for indices in zip(piece_indices,
                                                                                 [piece_index_range[0] for
                                                                                  piece_index_range in
                                                                                  piece_index_ranges])])
                                ...
                                piece_write_slices = tuple([
                                    (
                                        slice(
                                            np.count_nonzero(
                                                variable_masks[dimension_index][input_variable_slices[dimension_index]][
                                                :piece_read_slices[dimension_index].start]),
                                            (np.count_nonzero(
                                                variable_masks[dimension_index][input_variable_slices[dimension_index]][
                                                :piece_read_slices[dimension_index].start]) +
                                             np.count_nonzero(variable_masks[dimension_index][
                                                                  input_variable_slices[dimension_index]][
                                                                  piece_read_slices[dimension_index]]))
                                        )
                                        if variable_masks[dimension_index] is not None else
                                        slice(piece_read_slices[dimension_index].start - input_variable_slices[
                                            dimension_index].start,
                                              piece_read_slices[dimension_index].stop - input_variable_slices[
                                                  dimension_index].start
        ...
        if not self._netcdf_dataset:
            ...
            if self.opendap:
                try:
                    ...
                    self._netcdf_dataset = netCDF4.Dataset(self.nc_path + '#fillmismatch',
                                                           mode="r")  # Work-around for _FillValue mismatch: https://github.com/Unidata/netcdf-c/issues/1299
            else:
                self._netcdf_dataset = netCDF4.Dataset(self.nc_path, mode="r")

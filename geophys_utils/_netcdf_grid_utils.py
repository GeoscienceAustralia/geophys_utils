...
class NetCDFGridUtils(NetCDFUtils):
    ...
    def __init__(self, netcdf_dataset, debug=False):
        ...
        def set_nominal_pixel_sizes():
            ...
            self.y_inverted = (
                    self.y_variable[-1] < self.y_variable[0]).item()  # Should not have to deal with null values
            ...
            self.nominal_pixel_metres = [round(abs(centre_pixel_utm_coords[1][
                                                       dim_index] - centre_pixel_utm_coords[0][dim_index]), 8) for
                                         dim_index in range(2)]
            ...
            self.nominal_pixel_degrees = [round(abs(centre_pixel_wgs84_coords[1][
                                                        dim_index] - centre_pixel_wgs84_coords[0][dim_index]), 8) for
                                          dim_index in range(2)]
        ...
        try:
            ...
            logger.error(
                'Unable to determine data variable(s) (must have same dimensions as variable with "grid_mapping" attribute)')
            raise

        # TODO: Make this work for multi-variate grids
        assert len(
            self.data_variable_list) > 0, 'Unable to determine any data variables (must have same dimensions as variable with "grid_mapping" attribute)'
        ...
        type(self.pixel_count[0])
        type(self.pixel_count[1])
        ...
        self.native_bbox = [
            [self.GeoTransform[0] + (x_pixel_offset * self.GeoTransform[1]) + (y_pixel_offset * self.GeoTransform[2]),
    ...
    def get_indices_from_coords(self, coordinates, wkt=None):
        ...
        try:  # Multiple coordinates
            indices = [[np.where(
                abs(self.dimension_arrays[dim_index] - coordinate[dim_index]) <= (self.pixel_size[dim_index] / 2.0))[0][
                            0] for dim_index in range(2)]
                       if not ([True for dim_index in range(2) if
                                coordinate[dim_index] < self.min_extent[dim_index] or coordinate[dim_index] >
                                self.max_extent[dim_index]])
                       else None
                       for coordinate in native_coordinates]
        except TypeError:  # Single coordinate pair
            indices = ([np.where(abs(self.dimension_arrays[dim_index] - native_coordinates[dim_index]) <= (
                    self.pixel_size[dim_index] / 2.0))[0][0] for dim_index in range(2)]
                       if not [True for dim_index in range(2) if
                               native_coordinates[dim_index] < self.min_extent[dim_index] or native_coordinates[
                                   dim_index] > self.max_extent[dim_index]]
                       else None)
    ...
    def get_fractional_indices_from_coords(self, coordinates, wkt=None):
        ...
        try:  # Multiple coordinates
            fractional_indices = [
                [(coordinate[dim_index] - min(self.dimension_arrays[dim_index])) / self.pixel_size[dim_index] for
                 dim_index in range(2)]
                if not ([True for dim_index in range(2) if
                         coordinate[dim_index] < self.min_extent[dim_index] or coordinate[dim_index] > self.max_extent[
                             dim_index]])
                else None
                for coordinate in native_coordinates]
        except:  # Single coordinate pair
            fractional_indices = (
                [(native_coordinates[dim_index] - min(self.dimension_arrays[dim_index])) / self.pixel_size[dim_index]
                 for dim_index in range(2)]
                if not [True for dim_index in range(2) if
                        native_coordinates[dim_index] < self.min_extent[dim_index] or native_coordinates[dim_index] >
                        self.max_extent[dim_index]]
                else None)
    ...
    def get_concave_hull(self,
        ...
        buffer_distance = buffer_distance or max(
            self.data_variable.shape) // 20  # Tune this to suit overall size of grid
        # buffer_distance = 1000000
        logger.debug("buffer distance: {}".format(buffer_distance))
        ...
        def transform_geometry_pixel_to_wkt(geometry, to_wkt):
            ...
            if type(geometry) == MultiPolygon:
                return MultiPolygon([
                    Polygon(transform_coords(np.array([(affine_transform * pixel_coordinate_pair)
                ...
                return Polygon(transform_coords(np.array([(affine_transform * pixel_coordinate_pair)
        ...
        #     return Polygon(transform_coords(np.array(self.native_bbox), self.wkt, to_wkt))
        #
        ...
        for band in self.data_variable_list:
            ...
            padded_mask = np.zeros(
                shape=[int(ceil(size / downsampling_stride)) + 2 for size in self.data_variable.shape], dtype=bool)
        ...
        data_proportion = np.count_nonzero(padded_mask[1:-1, 1:-1]) / (
                (padded_mask.shape[0] - 2) * (padded_mask.shape[1] - 2))
        if data_proportion >= MAX_DATA_PROPORTION:
            logger.debug(
                'More than {:.2f}% of pixels contain data - assuming full grid coverage'.format(data_proportion * 100))
            return (transform_coords(np.array(self.native_bbox), self.wkt, to_wkt))
        ...
        contours = [(contour - PAD_WIDTH) * downsampling_stride
                    # Shift for padding and compensate for downsampling_stride
        ...
        concave_hull = get_offset_geometry(concave_hull, buffer_distance, offset, tolerance, cap_style, join_style,
                                           max_polygons, max_vertices)
    ...
    def get_dimension_ranges(self, bounds, bounds_wkt=None):
        ...
        try:
            ...
            for dim_index in range(2):
                ...
                subset_indices = \
                    np.where(np.logical_and(self.dimension_arrays[1 - dim_index] >= bounds_ordinates[dim_index],
                                            self.dimension_arrays[1 - dim_index] <= bounds_ordinates[dim_index + 2]))[0]

                logger.debug('subset_indices = {}'.format(subset_indices))

                logger.debug('self.dimension_arrays[{}].shape = {}'.format(1 - dim_index,
                                                                           self.dimension_arrays[1 - dim_index].shape))
                ...
                logger.debug('dim_range_dict["{}"] = {}'.format(dimension_names[1 - dim_index],
                                                                dim_range_dict[dimension_names[1 - dim_index]]))
    ...
    def copy(self,
        ...
        try:
            ...
            if dim_range_dict:  # Subsets were requested
                ...
                for gt_ordinate_index, ordinate_variable, gt_pixel_width_index in [(0, new_ncgu.x_variable, 1),
                                                                                   (3, new_ncgu.y_variable, 5)]:
                    if ordinate_variable.dimensions[0] in dim_range_dict.keys():
                        geo_transform[gt_ordinate_index] = ordinate_variable[0].item() - (
                                geo_transform[gt_pixel_width_index] / 2.0)  # Subtract half pixel width
            ...
            if invert_y == new_ncgu.y_inverted:
                logger.debug(
                    '{} does not require any alteration to make y-axis inversion {}'.format(nc_out_path, invert_y))
                return
    ...
    def set_global_attributes(self, compute_shape=True,
        ...
        try:
            ...
            metadata_bbox = get_reprojected_bounds(self.bounds, self.wkt,
                                                   gda_wkt)  # Reproject bounding box from native CRS to metadata CRS
            ...
            if compute_shape:
                ...
                # wkt_polygon = shapely.wkt.dumps(self.get_concave_hull(to_wkt=gda_wkt,
                wkt_polygon = shapely.wkt.dumps(Polygon(self.get_concave_hull(to_wkt=gda_wkt,
    ...
    def set_variable_actual_range_attribute(self, iterate_through_data=False, num_pixels_to_trigger_iterating=5000000,
        ...
        for variable_name, variable in self.netcdf_dataset.variables.items():
            ...
            if (iterate_through_data and self.pixel_count[0] * self.pixel_count[1] > num_pixels_to_trigger_iterating):
                logger.debug(
                    "Pixel count is greater than array_total_size setting. Iterating through variable to find min, max")
                min, max = self.iterate_through_data_chunks_and_find_mins_and_maxs(variable=variable,
...
def main():
    ...
    parser.add_argument("--complevel", help="Compression level for chunked variables as an integer 0-9. Default is 4",
                        type=int, default=4)
    parser.add_argument('-i', '--invert_y', help='Store copy with y-axis indexing Southward positive (grids only)',
                        type=str)

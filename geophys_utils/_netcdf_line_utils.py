...
DEFAULT_VAR_OPTIONS = {'complevel': 5,
                       #'zlib': True, # depcrecated replaced with compression='zlib'
                       'compression': 'zlib',
                       'fletcher32': True,
                       'shuffle': True,
                       'endian': 'little',
...
class NetCDFLineUtils(NetCDFPointUtils):
    ...
    def get_line_masks(self, line_numbers=None, subset_mask=None, get_contiguous_lines=False):
        ...
        if subset_mask is not None:
            line_number_subset = line_number_subset[np.isin(line_number_subset, self.line[
                np.unique(self.line_index[subset_mask])])]  # Exclude lines not in subset
        else:
            line_number_subset = line_number_subset[np.isin(line_number_subset, self.line)]  # Exclude bad line numbers

        line_mask = np.zeros(shape=self.line_index.shape, dtype=bool)  # Keep re-using same in-memory array
    ...
    def get_cached_line_arrays(self):
        ...
        if self.enable_disk_cache:
            ...
            if line is None or line_index is None:
                ...
                cache_dataset.close()
                logger.debug('Saved {} lines for {} points to cache file {}'.format(line.shape[0], line_index.shape[0],
                                                                                    self.cache_path))
        ...
        try:
            ...
            if compute_median_sample_spacing:
                try:
                    ...
                    sample_distance_array = coords2distance(
                        utm_coordinate_array)  # Absolute distances of sample points along line
                    sample_distance_array = sample_distance_array[1:] - sample_distance_array[
                                                                        :-1]  # Compute relative distances from absolute distances
    ...
    def fix_missing_coordinates(self):
        ...
        def interpolate(last_good_coord_index, next_good_coord_index, bad_point_count):
            ...
            def set_missing_ordinates(interpolated_point_array, start_index, flag_index_value):
                ...
                assert np.all(
                    np.any(np.isnan(self.xycoords[update_slice]), axis=1)), 'Unable to overwrite good coordinates'
                self.xycoords[
                    update_slice] = interpolated_point_array  # TODO: Check this - it could be dangerous setting a property
            ...
            logger.debug('last_good_coord_index = {}, next_good_coord_index = {}, bad_point_count = {}'.format(
                last_good_coord_index, next_good_coord_index, bad_point_count))
            if last_good_coord_index is not None:
                logger.debug('self.xycoords[self.xycoords[last_good_coord_index] = {}'.format(
                    self.xycoords[last_good_coord_index]))
            if next_good_coord_index is not None:
                logger.debug('self.xycoords[self.xycoords[next_good_coord_index] = {}'.format(
                    self.xycoords[next_good_coord_index]))
            ...
            if last_good_coord_index is not None and next_good_coord_index is not None:  # Interpolation between points required
                for interpolated_point_index in range(1, bad_point_count + 1):
                    interpolated_point_array[interpolated_point_index - 1] = (
                                                                                     (
                                                                                                 bad_point_count - interpolated_point_index + 1) *
                                                                                     self.xycoords[
                                                                                         last_good_coord_index]
                                                                                     + interpolated_point_index *
                                                                                     self.xycoords[
                                                                                         next_good_coord_index]) / (
                                                                                         bad_point_count + 1)
                ...
                raise BaseException(
                    'Need to provide at least one of last_good_coord_index and/or next_good_coord_index')
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    'Missing coordinates already interpolated and/or extrapolated (coordinate flag variables already exist)')
                return
            ...
            logger.debug('{} invalid coordinates found in {} blocks'.format(np.count_nonzero(bad_coord_mask),
                                                                            bad_coord_start_indices.shape[0]))
            ...
            for bad_range_index in range(len(bad_coord_start_indices)):
                ...
                logger.debug(
                    'bad_coord_start_index = {}, bad_line_end_index = {}, bad_line_start_index = {}, bad_coord_end_index = {}'.format(
                ...
                logger.debug('Extended missing coordinate range:\n{}'.format(
                    self.xycoords[bad_coord_start_index - 1:bad_coord_end_index + 2]))

                logger.debug('Extended Line numbers for missing points:\n{}'.format(
                    self.netcdf_dataset.variables['line'][:][(
                    self.netcdf_dataset.variables['line_index'][bad_coord_start_index - 1:bad_coord_end_index + 2])]))
            ...
            for flag_index in range(len(COORDINATE_FLAG_LIST)):
                logger.debug('{} coordinate count = {}'.format(COORDINATE_FLAG_LIST[flag_index], np.count_nonzero(
                    coordinate_flag_index_variable[:] == flag_index)))

...
class TileGridder(object):
    ...
    def __init__(self,
        ...
        self.grid_crs_wkt = get_wkt_from_spatial_ref(
            get_spatial_ref_from_wkt(grid_crs_wkt or TileGridder.GDA94_CRS_WKT))
        ...
        if tile_extra is None:  # Expand bounds by TileGridder.DEFAULT_TILE_EXPANSION_PERCENT percent each side
            self.expanded_grid_bounds = [
                grid_bounds[0] - (grid_bounds[2] - grid_bounds[0]) * TileGridder.DEFAULT_TILE_EXPANSION_PERCENT,
    ...
    def dataset_values(self):
        ...
        if self._dataset_values is None:
            self._dataset_values = {dataset: dataset_value_dict
                                    ...
                                    self.dataset_value_generator([self.grid_variable_name, self.filter_variable_name],
                                                                 self.dataset_list,
    ...
    def filter_points(self):
        ...
        for dataset in sorted(self.dataset_values.keys()):
            ...
            if len(coordinates):
                ...
                self.dataset_values[dataset][self.grid_variable_name][filter_mask]
        ...
        for dataset in dataset_list:
            try:
                try:
                    ...
                    nc_dataset = Dataset(
                        dataset + '#fillmismatch')  # Note work-around for bad _FillValue: https://github.com/Unidata/netcdf-c/issues/1299
                ...
                print('{}/{} points found in expanded bounding box for {}'.format(point_count,
                                                                                  netcdf_point_utils.point_count,
                                                                                  dataset))
                ...
                dataset_value_dict = {'coordinates': transform_coords(netcdf_point_utils.xycoords[spatial_mask],
                                                                      get_wkt_from_spatial_ref(get_spatial_ref_from_wkt(
                                                                          netcdf_point_utils.wkt)),
                                                                      self.grid_crs_wkt
                                                                      )
                                      }
                ...
                for variable_name in variable_name_list:
                    ...
                    if (variable.dimensions[0] != 'point'):  # Variable is NOT of point dimension - must be lookup
                        dataset_value_dict[variable_name] = netcdf_point_utils.expand_lookup_variable(
            ...
            for dataset in sorted(self.dataset_values.keys()):
                ...
                for point_index in range(len(arrays['coordinates'])):
                    output_file.write(','.join([os.path.basename(dataset),
                                                ','.join(
                                                    [str(ordinate) for ordinate in arrays['coordinates'][point_index]]),
...
def main():
    ...
    parser.add_argument("-b", "--bounds",
                        help='comma-separated <minx>,<miny>,<maxx>,<maxy> ordinates of bounding box for gridding. N.B: A leading "-" sign on this list should NOT be preceded by a space',
                        type=str, required=True)
    parser.add_argument("-t", "--tilesize", help="single value or comma-separated pair of x,y values for tile size",
                        type=str, required=True)
    ...
    parser.add_argument("-c", "--crs",
                        help='coordinate reference system for bounding box coordinates for search. Defaults to "EPSG:4283".',
                        type=str, default='EPSG:4283')
    parser.add_argument("-f", "--filter_variable", help='name of filter variable. Defaults to "gridflag"', type=str,
                        default='gridflag')
    ...
    parser.add_argument("-x", "--tile_extra",
                        help='absolute extra per side for tiling. Defaults to 5% extra on each side', type=float,
                        required=False)
    ...
    grid_bounds = [round(float(ordinate.strip()) / grid_resolution) * grid_resolution for ordinate in
                   args.bounds.split(',')]
    ...
    for ll_point in itertools.product(
            *[np.arange(grid_bounds[0 + dim_index], grid_bounds[2 + dim_index], tile_size[dim_index]) for dim_index in
              range(2)]):
        tile_bounds = [ll_point[0], ll_point[1], ll_point[0] + tile_size[0], ll_point[1] + tile_size[1]]
        print(tile_bounds)

        tile_path = os.path.join(output_dir, 'tiles',
                                 '_'.join([grid_variable_name] + [str(ordinate) for ordinate in tile_bounds]) + '.tif')
        point_list_path = os.path.join(output_dir, 'point_lists', '_'.join(
            [grid_variable_name] + [str(ordinate) for ordinate in tile_bounds]) + '.csv')
        dataset_list_path = os.path.join(output_dir, 'dataset_lists', '_'.join(
            [grid_variable_name] + [str(ordinate) for ordinate in tile_bounds]) + '.txt')
